"""
France Magento Database Client

Fetches invoices and order data directly from FR and NL Magento databases.
...
class FranceMagentoDBClient:
    """
    Client to interact with France and Netherlands Magento Databases directly
    
    For order approvals: Fetches from both FR and NL databases (both ship from France)
    For invoice lookup: Tries FR first, then NL
    ...
    def _get_connection(self, region: str = "fr"):
        """Get connection for specified region (fr or nl)"""
        conn = get_magento_connection(region=region)
        ...
        try:
            ...
            logger.warning(f"Failed to set read-only mode for {region}: {e}")
    ...
    def get_invoice_by_order_number(self, order_number: str) -> Optional[MagentoInvoice]:
        ...
        Tries FR first, then NL
        """
        # Try FR first
        invoice = self._get_invoice_by_order_number_from_region(order_number, "fr")
        if invoice:
            return invoice
        
        # Try NL if not found in FR
        return self._get_invoice_by_order_number_from_region(order_number, "nl")
    
    def _get_invoice_by_order_number_from_region(self, order_number: str, region: str) -> Optional[MagentoInvoice]:
        """Fetch invoice from a specific region's database"""
        ...
        try:
            with conn.cursor() as cursor:
                ...
                return self._build_invoice_model(conn, invoice, order, region)
        finally:
            conn.close()
    ...
    def get_invoice_by_invoice_number(self, invoice_number: str) -> Optional[MagentoInvoice]:
        ...
        """
        # Try FR first
        invoice = self._get_invoice_by_invoice_number_from_region(invoice_number, "fr")
        if invoice:
            return invoice
        
        # Try NL if not found
        return self._get_invoice_by_invoice_number_from_region(invoice_number, "nl")
    
    def _get_invoice_by_invoice_number_from_region(self, invoice_number: str, region: str) -> Optional[MagentoInvoice]:
        """Fetch invoice by invoice number from a specific region"""
        conn = self._get_connection(region)
        try:
            with conn.cursor() as cursor:
                # 1. Find invoice
    ...
    def _build_invoice_model(self, conn, invoice: Dict, order: Optional[Dict], region: str) -> MagentoInvoice:
        ...
        with conn.cursor() as cursor:
            ...
            return MagentoInvoice(
                ...
                source_region=region  # Track which region this came from
            )
    ...
    def get_processing_orders(self, limit: int = 50, status: str = 'processing') -> List[Dict[str, Any]]:
        """
        Get orders in processing status from BOTH FR and NL Magento databases.
        Both regions ship from France, so we combine them.
        """
        all_orders = []
        
        # Fetch from FR
        fr_orders = self._get_processing_orders_from_region("fr", limit, status)
        for order in fr_orders:
            order['source_region'] = 'fr'
        all_orders.extend(fr_orders)
        
        # Fetch from NL
        nl_orders = self._get_processing_orders_from_region("nl", limit, status)
        for order in nl_orders:
            order['source_region'] = 'nl'
        all_orders.extend(nl_orders)
    ...
    def _get_processing_orders_from_region(self, region: str, limit: int, status: str) -> List[Dict[str, Any]]:
        """Get processing orders from a specific region's database"""
        conn = self._get_connection(region)
        try:
            with conn.cursor() as cursor:
...
# Singleton instance
_client: Optional[FranceMagentoDBClient] = None


def get_france_magento_client() -> FranceMagentoDBClient:
    """Get or create France Magento client instance"""
    global _client
    if _client is None:
        _client = FranceMagentoDBClient()
    return _client
