"""
Database-backed repository for managing London Orders fulfillment sessions.
...
Inventory operations use the uk_london_inventory table.
"""
...
# London branch inventory table
LONDON_INVENTORY_TABLE = 'uk_london_inventory'

# London session tables
LONDON_SESSIONS_TABLE = 'london_order_fulfillment_sessions'
LONDON_TAKEOVER_TABLE = 'london_order_fulfillment_takeover_requests'


def init_london_fulfillment_tables():
    ...
    try:
        ...
        cursor.execute("""
            ...
            BEGIN
                ...
                IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'order_fulfillment_sessions')
                   AND NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'london_order_fulfillment_sessions') THEN
                    ALTER TABLE order_fulfillment_sessions RENAME TO london_order_fulfillment_sessions;
                    RAISE NOTICE 'Renamed order_fulfillment_sessions to london_order_fulfillment_sessions';
                END IF;
                ...
                IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'order_fulfillment_takeover_requests')
                   AND NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'london_order_fulfillment_takeover_requests') THEN
                    ALTER TABLE order_fulfillment_takeover_requests RENAME TO london_order_fulfillment_takeover_requests;
                    RAISE NOTICE 'Renamed order_fulfillment_takeover_requests to london_order_fulfillment_takeover_requests';
                END IF;
            END $$;
        """)
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {LONDON_SESSIONS_TABLE} (
        ...
        cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS {LONDON_TAKEOVER_TABLE} (
                request_id UUID PRIMARY KEY,
                session_id UUID REFERENCES {LONDON_SESSIONS_TABLE}(session_id) ON DELETE CASCADE,
        ...
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_sessions_order_number 
            ON {LONDON_SESSIONS_TABLE}(order_number)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_sessions_status 
            ON {LONDON_SESSIONS_TABLE}(status)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_sessions_user_id 
            ON {LONDON_SESSIONS_TABLE}(user_id)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_sessions_started_at 
            ON {LONDON_SESSIONS_TABLE}(started_at)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_sessions_invoice_id 
            ON {LONDON_SESSIONS_TABLE}(invoice_id)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_takeover_session_id 
            ON {LONDON_TAKEOVER_TABLE}(session_id)
        """)
        cursor.execute(f"""
            CREATE INDEX IF NOT EXISTS idx_london_takeover_current_owner 
            ON {LONDON_TAKEOVER_TABLE}(current_owner)
        """)
        
        # Migration: Add items_counted column if it doesn't exist (for existing databases)
        cursor.execute(f"""
            ALTER TABLE {LONDON_SESSIONS_TABLE} 
...
def check_london_fulfillment_tables_exist() -> dict:
    ...
    try:
        ...
        tables = [
            LONDON_SESSIONS_TABLE,
            LONDON_TAKEOVER_TABLE
        ]
        status = {}
...
class LondonDbRepo:
    ...
    def get_sku_by_item_id(self, item_id: str) -> Optional[str]:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT sku FROM {LONDON_INVENTORY_TABLE} WHERE item_id = %s",
                (item_id,)
            )
    ...
    def _add_audit_log(self, conn, session_id: str, action: str, user: str, details: Optional[str] = None):
        ...
        cursor.execute("""
            UPDATE london_order_fulfillment_sessions 
    ...
    def create_session(self, 
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO london_order_fulfillment_sessions 
    ...
    def get_session(self, session_id: str) -> Optional[ScanSession]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT * FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def get_active_session_for_invoice(self, invoice_id: str) -> Optional[ScanSession]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT * FROM london_order_fulfillment_sessions 
    ...
    def reactivate_session(self, session_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                SELECT status, audit_logs FROM london_order_fulfillment_sessions 
                WHERE session_id = %s
            """, (session_id,))
            row = cursor.fetchone()
            if not row:
    ...
    def add_scanned_item(self, session_id: str, sku: str, quantity: float, deduction_sources: list = None) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                SELECT items_scanned FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def complete_session(self, session_id: str, user_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                SELECT user_id, last_modified_by FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def restart_cancelled_session(self, session_id: str, user_id: Optional[str] = None) -> Optional[ScanSession]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT status FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def claim_session(self, session_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                SELECT status, created_by FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def release_session(self, session_id: str, user_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Get current session info
            cursor.execute("""
                SELECT user_id FROM london_order_fulfillment_sessions WHERE session_id = %s
            """, (session_id,))
    ...
    def create_takeover_request(self, session_id: str, requested_by: str) -> Optional[TakeoverRequest]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT * FROM london_order_fulfillment_takeover_requests 
            ...
            cursor.execute("""
                INSERT INTO london_order_fulfillment_takeover_requests 
            ...
            cursor.execute("""
                SELECT * FROM london_order_fulfillment_takeover_requests WHERE request_id = %s
            """, (request_id,))
    ...
    def respond_to_takeover_request(self, request_id: str, accept: bool) -> Optional[TakeoverRequest]:
        ...
        try:
            ...
            cursor.execute("""
                UPDATE london_order_fulfillment_takeover_requests 
    ...
    def reset_daily_sessions(self) -> dict:
        ...
        try:
            ...
            cursor.execute("""
                SELECT status, COUNT(*) FROM london_order_fulfillment_sessions 
            ...
            cursor.execute("""
                ...
                FROM london_order_fulfillment_sessions 
                WHERE status != 'archived'
            """)
            ...
            cursor.execute("DELETE FROM london_order_fulfillment_takeover_requests")
