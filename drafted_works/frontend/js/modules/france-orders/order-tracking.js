...
export function cleanup() {
  console.log('[France Order Tracking] Cleaning up...');
...
function initializeWebSocket() {
  ...
  if (currentUser && currentUser.username) {
    wsService.connect(currentUser).then(() => {
      console.log('[France Order Tracking] WebSocket connected, joining france_orders room');
      ...
      console.error('[France Order Tracking] WebSocket connection failed:', error);
      if (liveStatus) {
        liveStatus.classList.remove('connected');
        liveStatus.querySelector('.status-text').textContent = 'Offline';
      }
...
async function loadTrackingBoard() {
  try {
    const response = await get('/v1/france-magento/tracking/board');
...
async function cancelSession(sessionId, modal) {
  try {
    const result = await post(`/api/v1/france-magento/sessions/${sessionId}/cancel`, {});
...
async function releaseSession(sessionId, modal) {
  try {
    const result = await post(`/api/v1/france-magento/sessions/${sessionId}/release`, {});
...
async function showOrderDetails(order) {
  ...
  let invoice = null;
  try {
    invoice = await get(`/v1/france-magento/invoice/lookup/${order.order_number}`);
  } catch (error) {
  ...
  if (order.status === 'ready_to_check') {
    ...
    actionBtn.onclick = () => {
      modal.classList.remove('active');
      navigate(`/france-orders/order-fulfillment/session-${order.session_id}`);
    };
