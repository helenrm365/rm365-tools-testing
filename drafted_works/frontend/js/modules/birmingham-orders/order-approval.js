...
class OrderApprovalManager {
  ...
  async setupWebSocket() {
    ...
    if (currentUser && currentUser.username) {
      try {
        if (!wsService.isConnected()) {
          ...
          console.log('[Birmingham Order Approval] WebSocket connected');
        }
        wsService.joinRoom('birmingham_orders');
        console.log('[Birmingham Order Approval] Joined birmingham_orders room');
        ...
        console.error('[Birmingham Order Approval] WebSocket connection failed:', error);
        const liveStatus = document.getElementById('liveStatus');
        if (liveStatus) {
          liveStatus.classList.remove('connected');
          liveStatus.querySelector('.status-text').textContent = 'Offline';
    ...
    console.log('[Birmingham Order Approval] WebSocket listeners set up for real-time updates');
  }
  ...
  cleanup() {
    console.log('[Birmingham Order Approval] Cleaning up...');
