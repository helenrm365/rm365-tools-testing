...
async function loadTrackingBoard() {
  try {
    const response = await get('/v1/london-magento/tracking/board');
...
async function cancelSession(sessionId, modal) {
  try {
    const result = await post(`/api/v1/london-magento/sessions/${sessionId}/cancel`, {});
...
async function releaseSession(sessionId, modal) {
  try {
    const result = await post(`/api/v1/london-magento/sessions/${sessionId}/release`, {});
...
async function showOrderDetails(order) {
  ...
  let invoice = null;
  try {
    invoice = await get(`/v1/london-magento/invoice/lookup/${order.order_number}`);
  } catch (error) {
  ...
  if (order.status === 'ready_to_check') {
    ...
    actionBtn.onclick = () => {
      modal.classList.remove('active');
      navigate(`/london-orders/order-fulfillment/session-${order.session_id}`);
    };
