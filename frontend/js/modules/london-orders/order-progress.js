    ...
    if (currentUser && currentUser.username) {
        try {
            if (!wsService.isConnected()) {
                ...
                console.log('[London Order Progress] WebSocket connected');
            }
            wsService.joinRoom('london_orders');
            console.log('[London Order Progress] Joined london_orders room');
            wsConnected = true;
        } catch (error) {
            console.error('[London Order Progress] WebSocket connection failed:', error);
        }
    }
...
export function cleanup() {
    console.log('[London Order Progress] Cleaning up...');
...
async function loadSessions(showLoading = true) {
    ...
    try {
        const url = `${getApiUrl()}/v1/london-magento/dashboard/sessions?include_completed=${includeCompleted}`;
...
async function executeForceCancel(sessionId, reason) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/london-magento/dashboard/sessions/${sessionId}/force-cancel`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
...
async function executeForceAssign(sessionId, targetUser) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/london-magento/dashboard/sessions/${sessionId}/force-assign`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
...
async function executeTakeover(sessionId, session) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/london-magento/dashboard/sessions/${sessionId}/takeover`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
        ...
        if (result.order_number && result.invoice_number) {
            const path = `/london-orders/order-fulfillment/session-${result.order_number}-${result.invoice_number}`;
            if (window.navigate) {
