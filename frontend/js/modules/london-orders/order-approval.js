...
class OrderApprovalManager {
  ...
  async loadPendingOrders() {
    try {
      ...
      console.log('[Order Approval] API endpoint: /v1/london-magento/tracking/pending-orders');
      ...
      const response = await get('/v1/london-magento/tracking/pending-orders');
      ...
      if (response && response.orders) {
        ...
        if (this.pendingOrders.length === 0) {
          ...
          try {
            ...
            const debugResponse = await get('/v1/london-magento/tracking/pending-orders/debug');
  ...
  async approveOrder(orderId) {
    try {
      ...
      post('/v1/london-magento/tracking/approve-order', {
  ...
  async approveAllOrders() {
    ...
    for (const order of this.pendingOrders) {
      try {
        await post('/v1/london-magento/tracking/approve-order', {
          order_number: order.order_number
        });
        successCount++;
      } catch (error) {
