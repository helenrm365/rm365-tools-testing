...
import { getNLMagentoData, getNLAggregatedData, getCustomRangeAggregatedData, refreshAggregatedDataForRegion, checkTablesStatus, initializeTables, syncNLMagentoData } from '../../services/api/magentoDataApi.js?v=7';
...
async function loadMagentoData() {
  ...
  try {
    ...
    if (viewMode === 'aggregated') {
      ...
      result = await getNLMagentoData(pageSize, offset, '', currentSortColumn || '', currentSortDirection);
    }
...
async function loadSearchResults(searchTerm) {
  ...
  try {
    ...
    if (viewMode === 'aggregated') {
      ...
      result = await getNLMagentoData(pageSize, offset, searchTerm, currentSortColumn || '', currentSortDirection);
    }
...
async function fetchAllDataForExport() {
  ...
  while (hasMore) {
    ...
    if (viewMode === 'aggregated') {
      ...
      result = await getCustomRangeAggregatedData('nl', rangeType, rangeValue, useExclusions !== false, batchSize, offset, currentSearch);
    } else {
      break;
    }
