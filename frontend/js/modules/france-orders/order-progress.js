    ...
    if (currentUser && currentUser.username) {
        try {
            if (!wsService.isConnected()) {
                ...
                console.log('[France Order Progress] WebSocket connected');
            }
            wsService.joinRoom('france_orders');
            console.log('[France Order Progress] Joined france_orders room');
            wsConnected = true;
        } catch (error) {
            console.error('[France Order Progress] WebSocket connection failed:', error);
        }
    }
...
export function cleanup() {
    console.log('[France Order Progress] Cleaning up...');
...
async function loadSessions(showLoading = true) {
    ...
    try {
        const url = `${getApiUrl()}/v1/france-magento/dashboard/sessions?include_completed=${includeCompleted}`;
...
async function executeForceCancel(sessionId, reason) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/france-magento/dashboard/sessions/${sessionId}/force-cancel`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
...
async function executeForceAssign(sessionId, targetUser) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/france-magento/dashboard/sessions/${sessionId}/force-assign`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
...
async function executeTakeover(sessionId, session) {
    try {
        const response = await fetch(`${getApiUrl()}/v1/france-magento/dashboard/sessions/${sessionId}/takeover`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...getAuthHeaders()
        ...
        if (result.order_number && result.invoice_number) {
            const path = `/france-orders/order-fulfillment/session-${result.order_number}-${result.invoice_number}`;
            if (window.navigate) {
