// frontend/js/modules/france-orders/scanner.js
// France Orders - Inventory Scanner Module
// Uses FR Paris branch inventory (fr_paris_inventory table)
//
// Workflow Logic:
...
const BRANCH_CONFIG = {
  branchId: 'fr-paris',
  branchName: 'FR Paris',
  apiPrefix: '/v1/inventory/management/fr-paris',
  adjustmentsApiPrefix: '/v1/inventory/adjustments',
  scanningLogsApiPrefix: '/v1/inventory/scanning-logs/fr-paris'
};
...
export async function init(path) {
  console.log('[France Scanner] Initializing...');
  ...
  try {
    ...
    if (statusResult.status === 'success' && !statusResult.all_tables_exist) {
      ...
      console.log('[France Scanner] Some tables missing, initializing...', statusResult.tables_status);
      await initializeTables(BRANCH_CONFIG.branchId);
      console.log('[France Scanner] Tables initialized successfully');
    }
  } catch (error) {
    console.error('[France Scanner] Error checking/initializing tables:', error);
  ...
  console.log('[France Scanner] Initialized');
}

export async function cleanup() {
  console.log('[France Scanner] Cleaning up...');
