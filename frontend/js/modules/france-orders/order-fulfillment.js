// frontend/js/modules/france-orders/order-fulfillment.js
// France Orders - Order Fulfillment Module
// Uses France branch inventory (france_inventory table)
...
class MagentoPickPackManager {
  constructor(initialPath) {
    ...
    this.currentPath = initialPath || '/france-orders/order-fulfillment';
  ...
  async ensureRealtimeConnection() {
    try {
      ...
      console.log('[MagentoPickPack] Requested to join france_orders room');
    } catch (error) {
  ...
  cleanupWebSocket() {
    console.log('[France cleanupWebSocket] Removing WebSocket handlers');
    ...
    console.log('[France cleanupWebSocket] Handlers removed');
  ...
  async handleSessionAssigned(data) {
    ...
    if (orderNumber && sessionId) {
      try {
        ...
        const targetPath = `/france-orders/order-fulfillment/session-${orderNumber}-`;
        if (currentPath && currentPath.startsWith(targetPath)) {
          return;
        }
        
        // Load session status to retrieve invoice_number for deep-link
        const url = `${getApiUrl()}/v1/france-magento/session/status/${sessionId}`;
        const response = await fetch(url, { headers: getAuthHeaders() });
        if (response.ok) {
          const status = await response.json();
          if (status?.order_number && status?.invoice_number) {
            const path = `/france-orders/order-fulfillment/session-${status.order_number}-${status.invoice_number}`;
            if (window.navigate) {
  ...
  async checkSessionFromPath(path) {
    if (!path || path === '/france-orders/order-fulfillment') {
      // Base path, show order lookup (this is initial load)
      await this.showOrderLookup(true);
      return;
    }

    // Check if path matches session URL pattern: /france-orders/order-fulfillment/session-{order}-{invoice}
    const sessionMatch = path.match(/\/france-orders\/order-fulfillment\/session-([^-]+)-(.+)/);
    if (sessionMatch) {
      const orderNumber = sessionMatch[1];
      const invoiceNumber = sessionMatch[2];
      // Try to find and load this session
      try {
        const statusUrl = `${getApiUrl()}/v1/france-magento/session/check/${orderNumber}`;
  ...
  async startSession(orderNumber = null, sessionType = null) {
    ...
    try {
      ...
      const checkUrl = `${getApiUrl()}/v1/france-magento/session/check/${encodeURIComponent(order)}`;
      ...
      if (statusData.status === 'draft') {
        ...
        if (shouldCancelOrder) {
          ...
          try {
            const cancelUrl = `${getApiUrl()}/v1/france-magento/session/${statusData.session_id}/cancel`;
      ...
      const url = `${getApiUrl()}/v1/france-magento/session/start`;
      ...
      const sessionUrl = `/france-orders/order-fulfillment/session-${session.order_number}-${session.invoice_number}`;
  ...
  async claimSession(sessionId) {
    try {
      ...
      const url = `${getApiUrl()}/v1/france-magento/sessions/${sessionId}/claim`;
      const response = await fetch(url, {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
      ...
      const sessionUrl = `/france-orders/order-fulfillment/session-${this.currentSession.order_number}-${this.currentSession.invoice_number}`;
  ...
  async showOrderLookup(isInitialLoad = false) {
    console.log('[France showOrderLookup] Starting - hiding session, showing lookup');
    console.log('[France showOrderLookup] DOM elements:', {
    ...
    if (!this.activeSessionSection || !this.orderLookupSection) {
      console.error('[France showOrderLookup] CRITICAL: Missing DOM elements!');
    ...
    console.log('[France showOrderLookup] Section visibility updated:', {
    ...
    const baseUrl = '/france-orders/order-fulfillment';
  ...
  async loadTrackingBoard() {
    console.log('[France loadTrackingBoard] Starting...');
    // No loading screen - router handles initial load, WebSocket handles updates
    try {
      const url = `${getApiUrl()}/v1/france-magento/tracking/board`;
      console.log('[France loadTrackingBoard] Fetching from:', url);
      const response = await fetch(url, { headers: getAuthHeaders() });
      
      if (!response.ok) {
        console.error('[France loadTrackingBoard] Response not OK:', response.status, response.statusText);
        showToast(`API Error: ${response.status} ${response.statusText}`, 'error');
        throw new Error('Failed to load tracking board');
      }
      
      const data = await response.json();
      console.log('[France loadTrackingBoard] Data received:', {
      ...
      console.log('[France loadTrackingBoard] Complete');
      showToast(`Loaded ${(data.ready_to_pick?.length || 0) + (data.ready_to_check?.length || 0)} orders`, 'success');
    } catch (error) {
      console.error('[France Order Fulfillment] Error loading tracking board:', error);
  ...
  updateColumn(columnName, orders) {
    ...
    console.log('[France updateColumn]', columnName, '- columnEl:', !!columnEl, 'countEl:', !!countEl, 'orders:', orders.length);
    
    if (!columnEl || !countEl) {
      console.error('[France updateColumn] Missing DOM element for column:', columnName, 'columnEl:', !!columnEl, 'countEl:', !!countEl);
      return;
    }
  ...
  async startSessionFromCard(orderNumber, sessionType = 'pick', isDraft = false) {
    try {
      ...
      const url = `${getApiUrl()}/v1/france-magento/invoice/lookup/${orderNumber}`;
      const response = await fetch(url, { headers: getAuthHeaders() });
  ...
  async showCompletedSessionModal(order) {
    try {
      ...
      const checkUrl = `${getApiUrl()}/v1/france-magento/session/check/${order.order_number}`;
      ...
      const statusUrl = `${getApiUrl()}/v1/france-magento/session/status/${checkData.session_id}`;
  ...
  async navigateToSession(order) {
    const path = `/france-orders/order-fulfillment/session-${order.order_number}-${order.invoice_number}`;
  ...
  async _performScan(sku, quantity, field) {
    try {
      ...
      const response = await fetch(`${getApiUrl()}/v1/france-magento/session/scan`, {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json'
  ...
  async refreshSessionStatus() {
    ...
    try {
      const response = await fetch(`${getApiUrl()}/v1/france-magento/session/status/${this.currentSessionId}`, {
        headers: getAuthHeaders()
      });
  ...
  async completeSession() {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/v1/france-magento/session/complete`, {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json'
  ...
  async markReadyToCheck() {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/v1/france-magento/tracking/mark-ready-to-check`, {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json'
  ...
  async saveAsDraft() {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/v1/france-magento/sessions/${this.currentSessionId}/release`, {
        method: 'POST',
        headers: getAuthHeaders()
      });
  ...
  async cancelSession() {
    ...
    try {
      const response = await fetch(`${getApiUrl()}/v1/france-magento/session/${this.currentSessionId}`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      });
  ...
  async sendBackForPicking() {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/v1/france-magento/tracking/send-back-for-picking`, {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json'
  ...
  async loadActiveSessions() {
    try {
      const response = await fetch(`${getApiUrl()}/v1/france-magento/sessions/active`, {
        headers: getAuthHeaders()
      });
...
export async function init(path) {
  console.log('[France OrderFulfillment] init() called with path:', path);
  ...
  try {
    ...
    console.error('[France OrderFulfillment] Failed to check/initialize tables:', error);
  ...
  if (window.__magentoPickPackManager) {
    console.log('[France OrderFulfillment] Cleaning up existing manager');
    try {
      window.__magentoPickPackManager.cleanupWebSocket();
    } catch (e) {
      console.warn('[France OrderFulfillment] Error cleaning up previous manager:', e);
    }
  ...
  console.log('[France OrderFulfillment] Waiting for DOM with double RAF...');
  await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
  console.log('[France OrderFulfillment] DOM should be ready now');
  
  // Verify DOM elements exist
  const orderLookupSection = document.getElementById('orderLookupSection');
  const trackingBoard = document.getElementById('trackingBoard');
  console.log('[France OrderFulfillment] DOM check - orderLookupSection:', !!orderLookupSection, 'trackingBoard:', !!trackingBoard);
  
  if (!orderLookupSection) {
    console.error('[France OrderFulfillment] CRITICAL: orderLookupSection not found in DOM!');
  ...
  console.log('[France OrderFulfillment] Manager created, elements:', {
  ...
  console.log('[France OrderFulfillment] Calling ensureRealtimeConnection...');
  await manager.ensureRealtimeConnection();
  console.log('[France OrderFulfillment] WebSocket connection attempt complete');
  
  // Now load the tracking board data
  showToast('Loading tracking data...', 'info');
  console.log('[France OrderFulfillment] Calling checkSessionFromPath...');
  manager.initialLoadPromise = manager.checkSessionFromPath(path);
  await manager.initialLoadPromise;
  console.log('[France OrderFulfillment] init() complete');
}
...
export function cleanup() {
  console.log('[France OrderFulfillment] cleanup() called');
  // Cleanup WebSocket listeners if manager exists
  if (window.__magentoPickPackManager) {
    console.log('[France OrderFulfillment] Cleaning up WebSocket listeners');
    window.__magentoPickPackManager.cleanupWebSocket();
  }
  ...
  window.__magentoPickPackInitialized = false;
  window.__magentoPickPackManager = null;
  console.log('[France OrderFulfillment] cleanup() complete');
}
