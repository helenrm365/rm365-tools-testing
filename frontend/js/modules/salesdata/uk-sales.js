...
function setupEventListeners() {
  ...
  const clearSearch = () => {
    ...
    console.log('[UK Sales] Clearing search');
    currentSearch = '';
    currentPage = 0;
    applySearchFilter();
  };
  ...
  try {
    ...
    while (hasMore) {
      if (viewMode === 'condensed') {
        result = await getUKCondensedData(batchSize, offset, '');
      } else {
        result = await getUKSalesData(batchSize, offset, '');
      }
      ...
      if (result.status === 'success' && result.data && result.data.length > 0) {
        ...
        tbody.innerHTML = `<tr><td colspan="${colSpan}" style="text-align: center; padding: 2rem;">Loading UK Sales... (${allData.length} records loaded)</td></tr>`;
...
function applySearchFilter() {
  console.log('[UK Sales] Applying search filter:', currentSearch);
  ...
  if (currentSearch && currentSearch.length > 0) {
    ...
    console.log(`[UK Sales] No search - showing all ${allData.length} rows`);
  }
