...
function setupEventListeners() {
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', () => {
      ...
      console.log('[FR Sales] Search triggered with value:', searchValue);
  ...
  if (clearSearchBtn) {
    clearSearchBtn.addEventListener('click', () => {
      ...
      currentSearch = '';
      currentPage = 0;
      console.log('[FR Sales] Search cleared');
      loadSalesData();
    });
  }
  ...
  if (searchInput) {
    searchInput.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        const searchValue = searchInput.value?.trim() || '';
        console.log('[FR Sales] Search triggered (Enter key) with value:', searchValue);
        currentSearch = searchValue;
        currentPage = 0;
        loadSalesData();
      }
...
async function loadSalesData() {
  ...
  try {
    ...
    console.log(`[FR Sales] Loading data - Mode: ${viewMode}, Page: ${currentPage}, Search: "${currentSearch}"`);
    
    if (viewMode === 'condensed') {
      result = await getFRCondensedData(pageSize, offset, currentSearch);
      displayCondensedData(result.data, result.total_count);
    } else {
      result = await getFRSalesData(pageSize, offset, currentSearch);
      displaySalesData(result.data, result.total_count);
    }
    
    if (result.status === 'success') {
      console.log(`[FR Sales] Loaded ${result.data.length} records (${result.total_count} total)`);
